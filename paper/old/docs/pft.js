...
Path.prototype.snapTo = function(path, degree){
    ...
    var d = 5;
    var x = this.position.x + (targetR-selfR)*Math.cos(degree)+d*Math.cos(((targetR-selfR)/selfR)*degree);
    var y = this.position.y + (targetR-selfR)*Math.sin(degree)-d*Math.sin(((targetR-selfR)/selfR)*degree);
    
    var dr = new Path.Circle({position:new Point(x,y), radius:2, fillColor:"blue"});
